...
#'
#' @param A dataframe containing the metabolite data.
#'
#' @return A dataframe containing initial data plus relative amount calculated data within same condition sample replicates.
#'
#' @examples RelAmounts_250K(df = data2)
#'
#' @export
#'


RelAmounts_250K <- function (df)
{
  data4 <- df %>%
  ...
  data4$Amount <- suppressMessages(mapvalues(data4$Amount, c('Inf','NaN', 0), c(NA,NA,NA)))
