bar_update_manual <- function(metabolites, bar.type, repeats, n, type,index){
  ...
  if (sum(grepl('MID', names(bar.type))) >= 1){
    ...
    bar_plot_update(a, met, Title, x, y, axis.text.x, scales='free', num_cond = n, type = type,index)
  }
